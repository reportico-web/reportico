...
class ReporticoApp
{
    ...
    public function setUpReporticoSession()
    {
        // Get current session
     ...
     */
    public function closeReporticoSession()
    {
    }

    public function convertYMDtoLocal($in_time, $from_format, $to_format)
    {
    ...
    public function getDatepickerLanguage($in_format)
    {
        $retval = "";
        switch ($in_format) {
    ...
    public function getDatepickerFormat($in_format)
    {
        $retval = "";
        switch ($in_format) {
    ...
    public function parseDate($in_keyword, $in_time = false, $in_mask = "%d/%m/%Y")
    {
...
// -----------------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------------
    public function convertDateRangeDefaultsToDates($in_type, $in_string, &$range_start, &$range_end)
    {
        $retval = true;
        if ($in_type == "DATE") {
    ...
    public function getQueryColumn($name, &$arr)
    {
    ...
    public function getGroupColumn($name, &$arr)
    {
        foreach ($arr as $k => $val) {
    ...
    public function &get_db_image_string(
        $in_driver,
        $in_dbname,
        $in_hostname,
        $in_sql,
    ...
    public function keyValueInArray($in_arr, $in_key)
    {
    ...
    public function getRequestItem($in_val, $in_default = false, $in_default_condition = true)
    {
    ...
    public function sessionItem($in_item, $in_default = false)
    {
        $ret = false;
    ...
    public function sessionRequestItem($in_item, $in_default = false, $in_default_condition = true)
    {
        $ret = false;
        if (issetReporticoSessionParam($in_item)) {
            $ret = getReporticoSessionParam($in_item);
    ...
    public function getCheckboxValue($in_tag)
    {
    ...
    public function hhmmssToSeconds($in_hhmmss)
    {
...
// Debug Message Handler
    public function handleDebug($dbgstr, $in_level)
    {
...
// User Error Handler
    public function handleError($errstr, $type = E_USER_ERROR)
    {
        global $g_system_debug;
        global $g_code_area;
        global $g_errors;
...
// Look for a file in the include path, or the path of the current source file
    public function findFileToInclude($file_path, &$new_file_path, &$rel_to_include = "")
    {
...
// Translate string into another language using the (ReporticoApp::get("translations")) global array
    public function &swTranslate($in_string)
    {
...
// Translate string into another language using the (ReporticoApp::get("translations")) array
    public function &swTranslateReportDesc($in_report)
    {
        $in_report = preg_replace("/\.xml$/", "", $in_report);
        $out_string = false;
        if (ReportioApp::get("report_desc")) {
            if (array_key_exists(ReporticoApp::getConfig("language"), ReportioApp::get("report_desc"))) {
                $langset = &(ReportioApp::get("report_desc"))[ReporticoApp::getConfig("language")];
    ...
    public function loadModeLanguagePack($mode, $output_encoding = "utf-8", $replace = false)
    {
        $langfile = findBestLocationInIncludePath("language");

        // Look for encoding specific language file
        if (ReporticoApp::getConfig("SW_OUTPUT_ENCODING") && ReporticoApp::getConfig("SW_OUTPUT_ENCODING") != "UTF8" && is_dir($langfile . "/" . ReporticoApp::getConfig("language") . "/" . ReporticoApp::getConfig("SW_OUTPUT_ENCODING"))) {
            $langfile = $langfile . "/" . ReporticoApp::getConfig("language") . "/" . SW_OUTPUT_ENCODING . "/" . $mode . ".php";
            require $langfile;
        } else {
    ...
    public function loadProjectLanguagePack($project, $output_encoding = "utf-8")
    {
        ...
        if ((ReporticoApp::get("translations")) && is_array((ReporticoApp::get("translations"))) && isset((ReporticoApp::get("translations"))[ReporticoApp::getConfig("language")]) && is_array((ReporticoApp::get("translations"))[ReporticoApp::getConfig("language")])) {
            // Convert UTF-8 mode to output character set if differen from native language pack
            if (strtolower($output_encoding) != "utf-8") {
    ...
    public function localiseTemplateStrings(&$in_smarty, $in_template = "")
    {
    ...
    public function templateXlate($in_string)
    {
        if (!$in_string) {
            return $in_string;
        }
    ...
    public function swPathExecutable($in_path)
    {
    ...
    // of a file path
    public function findBestUrlInIncludePath($path)
    {
        $newpath = $path;
        $reltoinclude;
    ...
    public function findBestLocationInIncludePath($path)
    {
        $newpath = $path;
        $reltoinclude;
    ...
    public function buildForwardUrlGetParams($path, $forward_url_get_params, $remainder)
    {
    ...
    public function getLocaleDateFormat($in_format)
    {
    ...
    public function getReporticoUrlPath()
    {
        $newpath = "swutil.php";
        $found = findFileToInclude($newpath, $newpath, $reltoinclude);
        $newpath = getRelativePath(str_replace("/", "\\", realpath($newpath)), dirname(realpath($_SERVER["SCRIPT_FILENAME"])));
    ...
    public function dbCharsetToPhpCharset($in)
    {
        $out = $in;
        switch ($in) {
            case "None":$out = false;
    ...
    public function outputCharsetToPhpCharset($in)
    {
        $out = $in;
        switch ($in) {
            case "None":$out = false;
    ...
    public function getOutputEncodingHtml()
    {
    ...
    public function getRelativePath($path, $compareTo)
    {
    ...
    public function htmltorgbPchart($color)
    {
        if (is_array($color)) {
            return $color;
        }
    ...
    public function &loadExistingReport($reportfile, $projects_folder = "projects")
    {
        $q = new Reportico();
        global $g_project;
    ...
    to another
     */
    public function reformatDate($informat, $outformat, $date)
    {
    ...
    public function columnNameToLabel($columnname)
    {
     ...
     */
    public function issetReporticoSessionParam($param, $session_name = false)
    {
        if (!$session_name) {
     ...
     ** using current session namespace
     */
    public function setReporticoSessionParam($param, $value, $namespace = false, $array = false)
    {
        if (!$namespace) {
     ...
     ** using current session namespace
     */
    public function getReporticoSessionParam($param)
    {
     ...
     */
    public function existsReporticoSession()
    {
     ...
     ** using current session namespace
     */
    public function unsetReporticoSessionParam($param)
    {
        if (isset($_SESSION[ReporticoApp::get("session_namespace_key")][$param])) {
     ...
     */
    public function registerSessionParam($param, $value)
    {
     ...
     ** using current session namespace
     */
    public function reporticoSessionName()
    {
        //if ( ReporticoApp::get("session_namespace") )
     ...
     */
    public function reporticoNamespace()
    {
     ...
     **
     */
    public function initializeReporticoNamespace($namespace)
    {
