...
//echo "XML $lq->query_name $lq->criteria_display $lq->required $lq->criteria_list<BR>";
    ...
    public function generateWebService($in_report)
    {

        if (!preg_match("/(.*)\.xml/", $in_report, $matches)) {
            trigger_error(templateXlate("XMLCONFILE") . " $in_report " . templateXlate("XMLFORM"), E_USER_NOTICE);
            return;
        }
        ...
        $this->prepareWebServiceFile("wsdl.tpl", $wsdlfile, $stub);
        $this->prepareWebServiceFile("soapclient.tpl", $cltphpfile, $stub);
        $this->prepareWebServiceFile("soapserver.tpl", $srvphpfile, $stub);

    }

    public function prepareWebServiceFile($templatefile, $savefile = false, $instub)
    {
        $smarty = new \Smarty();
        ...
        $smarty->assign('PROJECT', ReporticoApp::getConfig("project"));
        ...
        if ($savefile) {
            ...
            echo "</PRE>";
            $this->writeReportFile($savefile, $data);
        } else {
    ...
    public function prepareWsdlData($savefile = false)
    {
        $smarty = new \S\marty();
        $smarty->compile_dir = findBestLocationInIncludePath("templates_c");
