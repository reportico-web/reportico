			...
			switch ( $anal["action"] )
			{
				...
				case "assg":
					...
					if ( $updates["DrilldownReport"] )
					{
                            ...
                            $content = "Drill";
                            $url = $this->query->get_action_url()."?xmlin=".$updates["DrilldownReport"]."&execute_mode=EXECUTE&target_format=HTML&target_show_body=1&project=".ReporticoApp::getConfig("project");
							$startbit= "'<a target=\"_blank\" href=\"".$this->query->get_action_url()."?xmlin=".$updates["DrilldownReport"]."&execute_mode=EXECUTE&target_format=HTML&target_show_body=1&project=".ReporticoApp::getConfig("project");
							$midbit = "";
							foreach ( $q->lookup_queries as $k => $v )
							{
					...
					case "CogModule":
                        ...
                        if ( get_request_item("partialMaintain", false ) )
		                    break;
		                		$text .= '<TR>';
						$text .= '<TD colspan="2">';
						$text .= '&nbsp;&nbsp;'.template_xlate('PROJECT').ReporticoApp::getConfig("project").'&nbsp;&nbsp;&nbsp;&nbsp;';
		...
		switch ( $type )
		{
			...
			case "FONTLIST":
				...
				if (is_dir($testpath)) 
				{
                    ...
                    trigger_error ( template_xlate("NOOPENDIR").$fontdir, E_USER_NOTICE );
