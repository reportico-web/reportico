...
class SqlParser
{
    ...
    public $unique = false;
    public $columnoffset = 0; // Positon in sql where select columns exists
    public $whereoffset = 0; // Positon in sql where WHERE clause exists or would exist
    public $haswhere = false; // Does the sql have a where clause?

    public function __construct($in_sql)
    {
        $this->sql = $in_sql;
    }

    public function importIntoQuery(&$in_query)
    {
    ...
    // **
    public function parse($warn_empty_aliases = true)
    {
        $err = false;
    ...
    public function tokeniseColumns($in_string)
    {
    ...
    // alias is added in
    // ----------------------------------------------------------------------------
    public function parseColumnList(&$original_sql, $in_string, $offset_in_original_sql, $warn_empty_aliases)
    {
    ...
    // ----------------------------------------------------------------------------
    public function parseColumn($in_colno, $in_string, &$auto_gen_alias, $warn_empty_aliases)
    {
        $err = false;
    ...
    // -----------------------------------------------------------------------------
    public function testQuery($in_query, $sql)
    {
        ...
        $sql = Assignment::reporticoMetaSqlCriteria($in_query->parent_query, $sql, false, false, "MAINTAIN");
