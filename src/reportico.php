...
class reportico_object
{
	...
	function & get_attribute ( $attrib_name )
		{
            ...
            if ( isset ( $this->attributes[$attrib_name] ) )
			    ...
			    {
				    $val = ReporticoApp::checkForDefaultConfig($attrib_name, $this->attributes[$attrib_name]);
				    return $val;
			    }
			    else
			    {
	...
	function & derive_meta_value ( $to_parse )
	{
        ...
        {
            $dbtype = ReporticoApp::getConfig("db_type");
...
class reportico extends reportico_object
{
	...
	var $pdf_engine_file = "ReportFPDF";
	...
	function &create_graph()
	{
        ...
        {
            case "NVD3":
		        $graph = new reportico_graph_nvd3($this, "internal");
                break;

            case "FLOT":
		        $graph = new reportico_graph_flot($this, "internal");
                break;

            case "JPGRAPH":
		       $graph = new reportico_graph_jgraph($this, "internal");
                break;

            case "PCHART":
            default:
		        $graph = new reportico_graph_pchart($this, "internal");
                break;
        }
    ...
    */
	...
	function login_check($smarty)
	{
		...
		if ( ReporticoApp::getConfig("project") == "admin" )
		{
			...
			if (isset_reportico_session_param('admin_password') || !ReporticoApp::isSetConfig ('admin_password') || ( ReporticoApp::isSetConfig ('admin_password_reset' ) && ReporticoApp::getConfig("admin_password_reset") == '' ) )
			{
				$loggedon = "ADMIN";
			}
			else
	...
	function build_query($in_is_expanding = false, $criteria_name = "", $in_design_mode = false, $no_warnings = false )
	{
	    ...
	    if ( $execute_mode != "MAINTAIN" )
	    {
		    $this->query_statement = Assignment::reportico_meta_sql_criteria($this->parent_query, $this->query_statement, false, $no_warnings, $execute_mode);
	    }
	...
	function initialize_panels($mode)
	{
		...
		$smarty->assign('AJAX_DATEPICKER_LANGUAGE', get_datepicker_language(ReporticoApp::get("language")));
		$smarty->assign('AJAX_DATEPICKER_FORMAT', get_datepicker_format(ReporticoApp::getConfig("prep_dateformat")));
	...
	function handle_xml_query_input($mode=false)
	{
        ...
        if ( $do_defaults)
        {
            ...
            if ( @file_exists($this->projects_folder."/".ReporticoApp::getconfig("project")."/reportico_defaults.php" ))
                include_once($this->projects_folder."/".ReporticoApp::getconfig("project")."/reportico_defaults.php");
	...
	function execute($mode=false, $draw=true)
	{
        ...
        if ( !exists_reportico_session() || isset($_REQUEST['clear_session']) || $this->clear_reportico_session)
            initialize_reportico_namespace(ReporticoApp::get("session_namespace_key"));
        ...
        // for later iconv use
        $this->db_charset = db_charset_to_php_charset(ReporticoApp::getConfig("db_encoding", "UTF8"));
        $this->output_charset = output_charset_to_php_charset(ReporticoApp::getConfig("output_encoding", "UTF8"));
...
//echo "set table ";
		...
		switch ($mode) 
		{
			...
			case "MODIFY":
                ...
                $status = $engine->perform_project_modifications(ReporticoApp::getConfig("project"));
                if ( $status["errstat"] != 0 )
                { 
			...
			case "EXECUTE":
				...
				if ( ( count($g_system_errors) > 0 || $g_debug_mode || count($g_system_debug) > 0 || !get_reportico_session_param("loggedin") ) )
				{
                    ...
                    // for rendering inside web pages .. in this case
                    // return_output_to_caller will be set to true
                    $template = $this->get_template_path('error.tpl');
	...
	function build_admin_screen()
	{
		...
		if ( ReporticoApp::get("static_menu") && is_array(ReporticoApp::get("static_menu")) )
		{
			...
			if ( $ct == 0 )
				handle_error( "No Menu Items Available - Check Language - ".ReporticoApp::get("language"));
	...
	function build_menu()
	{
		...
		$this->set_attribute("ReportTitle", ReporticoApp::getConfig("project_title", "Report Suite"));
	...
	function premaintain_query()
	{
		...
		{
			$nsql = Assignment::reportico_lookup_string_to_php($sql);
            $recordSet = false;
            $errorCode = false;
            $errorMessage = false;
            try {
	...
	function execute_query($in_criteria_name)
	{
		...
		foreach ( $this->pre_sql as $sql)
		{
			...
			$nsql = Assignment::reportico_meta_sql_criteria($this, $sql, true);
        ...
        if ( !$in_criteria_name )
        {
		    ...
		    if ( !$code || $code == "NONE" || $code == "XX" )
            {
                ...
                {
	                $source_path = find_best_location_in_include_path( $this->projects_folder."/".ReporticoApp::getConfig("project")."/".get_reportico_session_param("xmlin").".php" );
                }
                else
	                $source_path = find_best_location_in_include_path( $this->projects_folder."/".ReporticoApp::getConfig("project")."/".get_reportico_session_param("xmlin").".xml.php" );
...
function set_project_environment($initial_project = false, $project_folder = "projects", $admin_project_folder = "projects" )
{
    ...
    load_project_language_pack($project, output_charset_to_php_charset(ReporticoApp::getConfig("output_encoding", "UTF8")));
