...
{{ T_REPORT_FILE }} <input type="text" name="xmlout" id="swPrpSaveFile" value="{{ XMLFILE }}"> <input type="submit" class="{{ BOOTSTRAP_STYLE_PRIMARY_BUTTON }} swPrpSaveButton" type="submit" name="submit_xxx_SAVE" value="{{ T_SAVE }}">
    				   ...
    				   <div class="btn-group" role="group">
    				       ...
    				       <button type="submit" class="btn btn-default prepareMiniMaintain swMiniMaintain swMiniMaintainGroup" title="{{ T_EDIT }} {{ T_EDITSQL }}" id="submit_mainquerqury_SHOW" name="mainquerqurysqlt_QuerySql">
                                <span class="glyphicon glyphicon-pencil"></span>{{ T_EDITSQL }}
                            </button>
                            <button type="submit" class="btn btn-default prepareMiniMaintain swMiniMaintain swMiniMaintainGroup" title="{{ T_EDIT }} {{ T_EDITCOLUMNS }}" id="submit_mainquerquryqcol_SHOW" name="mainquerquryqcol_ANY">
                                <span class="glyphicon glyphicon-pencil"></span>{{ T_EDITCOLUMNS }}
                            </button>
                            <button type="submit" class="btn btn-default prepareMiniMaintain swMiniMaintain swMiniMaintainGroup" title="{{ T_EDIT }} {{ T_EDITASSIGNMENT }}" id="submit_mainquerassg" name="mainquerassg_ANY">
                                <span class="glyphicon glyphicon-pencil"></span>{{ T_EDITASSIGNMENT }}
                            </button>
                            <button type="submit" class="btn btn-default prepareMiniMaintain swMiniMaintain swMiniMaintainGroup" title="{{ T_EDIT }} {{ T_EDITGROUPS }}" id="submit_mainqueroutpgrps" name="mainqueroutpgrps_ANY">
                                <span class="glyphicon glyphicon-pencil"></span>{{ T_EDITGROUPS }}
                            </button>
                            <button type="submit" class="btn btn-default prepareMiniMaintain swMiniMaintain swMiniMaintainGroup" title="{{ T_EDIT }} {{ T_EDITGRAPHS }}" id="submit_mainqueroutpgrph" name="mainqueroutpgrph_ANY">
                                <span class="glyphicon glyphicon-pencil"></span>{{ T_EDITGRAPHS }}
                            </button>
                            <div class="btn btn-default swMiniMaintainGroupWithDropDown" role="group">
                        <li class="dropdown"><a class="btn dropdown-toggle swMiniMaintainGroupDropDown" data-toggle="dropdown" href="#">{{ T_MORE }}<span class="caret"></span></a>
                            <ul class="dropdown-menu reportico-dropdown">
                                ...
                                <li><input type="submit" class="{{ BOOTSTRAP_STYLE_TOOLBAR_BUTTON }}prepareMiniMaintain swMiniMaintain" style="margin-right: 30px" title="{{ T_EDIT }} {{ T_EDITDISPLAYORDER }}" id="submit_mainqueroutppghd0000form" value="{{ T_EDITDISPLAYORDER }}" name="mainqueroutpdord0000form_ANY"></li>
